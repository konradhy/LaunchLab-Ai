...
app.post("/writing/intro", async (req, res, next) => {
  try {
    ...
    let prompt =
      `The following tool creates an introduction paragraph for an article based on metadata provided:\n"""\n` +
      // Example 1
      `Title: Complete Introduction to the 30 Most Essential Data Structures & Algorithms\n` +
      `${audience ? `Audience: Programmers, Developers\n` : ``}` +
      `${
        desc
          ? `Description: An article that covers why it's important to know data structures and algorithms as a topic, even if its scary, and how they apply in technology, work, and life.\n`
          : ``
      }` +
      `${
        keywords
          ? `Keywords: Algorithms, Data, Structures, Coding, Learning\n`
          : ``
      }` +
      ...
      `${
        desc
          ? `Description: An article which covers ways to self brand, allowing you to be unique in a market that is already full. It makes the process possible without spending too much money while also building your own brand.\n`
          : ``
      }` +
      ...
      `${
        desc
          ? `Description: Exploring how the world is changing with the growth and use of AI that will have a bigger impact on day to day life now and in in the future.\n`
          : ``
      }` +
